...
namespace Quản_lý_điểm_sinh_vien_CNTT
{
    public partial class frmGiangVien : Form
    {
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            {   // Kiểm tra có bản ghi trong tblMON trích dẫn đến giảng viên hay không
                string selectCount = "SELECT COUNT(*) FROM tblMON WHERE MaGV='" + txtMaGV.Text + "'";
                SqlCommand cmdCount = new SqlCommand(selectCount, conn);
                int recordCount = Convert.ToInt32(cmdCount.ExecuteScalar());
                cmdCount.Dispose();
                if (recordCount > 0)
                {
                    // Có bản ghi trong tblMON trích dẫn đến giảng viên
                    MessageBox.Show("Không thể xóa giảng viên vì có bản ghi trong bảng tblMON trích dẫn đến giảng viên này!", "Cảnh báo", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                }
                else
                {
