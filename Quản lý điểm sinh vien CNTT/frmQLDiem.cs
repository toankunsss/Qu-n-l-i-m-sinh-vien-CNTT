...
namespace Quản_lý_điểm_sinh_vien_CNTT
{
    public partial class frmQLDiem : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            else if (dgrDiem.CurrentRow != null && dgrDiem.CurrentRow.Cells[0].Value != null && dgrDiem.CurrentRow.Cells[3].Value != null)
            {
                string currentMaSV = dgrDiem.CurrentRow.Cells[0].Value.ToString();
                string currentMonHoc = dgrDiem.CurrentRow.Cells[3].Value.ToString();

                if (txtMaSV.Text == currentMaSV && cboMonHoc.Text == currentMonHoc)
                {
                    MessageBox.Show("Sinh viên này đã được nhập điểm môn: " + currentMonHoc, "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    txtMaSV.Focus();
                }
            }
            else if (cboLop.Text == "")
